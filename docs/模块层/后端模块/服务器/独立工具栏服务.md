...
- **固定端口策略**: 使用5749端口，彻底解决动态端口带来的服务发现问题
- **即插即用**: 启动即可使用，无需复杂配置
...
#### SRPC WebSocket通信
- **端点**: `ws://localhost:5749`
- **协议**: Stagewise标准SRPC协议
- **功能**: 接收工具栏的prompt请求

#### 广播WebSocket通信
- **端点**: `ws://localhost:5749/broadcast`
- **协议**: 自定义JSON消息格式
...
```bash
# 健康检查
curl http://localhost:5749/health

# 服务状态
curl http://localhost:5749/api/toolbar/status

# Ping测试
curl http://localhost:5749/ping/stagewise
```
...
class ToolbarIntegration {
  ...
  connect() {
    this.ws = new WebSocket('ws://localhost:5749/broadcast');
...
class AdvancedToolbarIntegration {
  constructor(options = {}) {
    this.options = {
      url: 'ws://localhost:5749/broadcast',
...
async function checkToolbarServiceStatus() {
  try {
    const response = await fetch('http://localhost:5749/api/toolbar/status');
...
async function monitorClients() {
  try {
    const response = await fetch('http://localhost:5749/api/clients');
...
**问题**: 服务无法启动，提示端口占用
```bash
Error: listen EADDRINUSE: address already in use :::5749
```

**解决方案**:
```bash
# 查找占用端口的进程
netstat -tulpn | grep 5749
# 或者
lsof -i :5749
...
```
WebSocket connection failed: Error: connect ECONNREFUSED 127.0.0.1:5749
```
...
```bash
# 检查客户端连接状态
curl http://localhost:5749/api/clients

# 检查最新prompt状态
curl http://localhost:5749/api/latest-prompt

# 检查WebService连接端点
# 确保连接到 ws://localhost:5749/broadcast 而不是 ws://localhost:5749
```
...
```bash
# 使用netcat测试端口连通性
nc -zv localhost 5749

# 使用curl测试HTTP端点
curl -v http://localhost:5749/health

# 使用wscat测试WebSocket连接
npm install -g wscat
wscat -c ws://localhost:5749/broadcast
```
